...
- **Venice private reasoning**: Integration layer showing Venice as Scout's private analysis capability. FeedItem fields: `privateReasoningUsed`, `reasoningProvider`, `reasonSummary`, `disclosureSummary`. DemoState field: `privateReasoningEnabled` (boolean, default true, persisted). Surfaces in: Agent page "Reasoning & Privacy" card with real toggle + accordion, Activity feed violet "Venice-assisted" badge, Receipt "Reasoning Provenance" disclosure section. Toggle controls `privateReasoningEnabled` state; card stays visible in both on/off states with state-dependent copy. Venice is a capability, not a product takeover — no dedicated tab or mode toggle.
